...
Decision variables
------------------
x[0]  orientation   Real  [0, 175]  degrees
x[1]  angle_idx     Int   [0, 3]    index into ANGLES
...
from parking_solver.core.model import AisleDir, Layout, LayoutParams, LayoutType, Site, StallType
...
def _mean_walk_distance(layout: Layout, site: Site) -> float:
    ...
    for s in layout.stalls:
        cx, cy = s.polygon.centroid.coords[0]
        total += math.hypot(cx - ref.x, cy - ref.y)
    return total / len(layout.stalls)


def _ada_margin(layout: Layout, profile: RegulationProfile) -> float:
    """Fraction of required ADA stalls that are *provided above* requirement.

    Returns a value ≥ 0 when compliant; negative when deficient.
    Normalised so 0 = exactly met, 1 = double what's needed.
    """
...
class _ParkingProblem(ElementwiseProblem):
    def __init__(self, site: Site, profile: RegulationProfile, fixed=None):
        super().__init__(
            ...
            n_obj=N_OBJ,
            n_ieq_constr=0,
            xl=np.array([0.0, 0.0, 2.20]),
            xu=np.array([175.0, 3.0, 3.00]),
        )
        self._site = site
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        try:
            ...
            out["F"] = np.array([1e9, 1e9, 1e9, 1e9])
            return

        total = layout.metrics.total_stalls
        area_per_stall = layout.metrics.gross_area_per_stall if total else 1e9
        ada_m = _ada_margin(layout, self._profile)
        walk = _mean_walk_distance(layout, self._site)

        out["F"] = np.array([
            -float(total),          # minimise negative count
            float(area_per_stall),
            -float(ada_m),          # minimise negative margin
            float(walk),
        ])
...
def run(
    ...
    *generation_callback* is called after each generation with
    ``(gen_number, list[Candidate])``.  Use it to stream results to the UI.
    """
    ...
    if result.history and generation_callback:
        for gen_idx, algo_gen in enumerate(result.history):
            try:
                pop = algo_gen.opt
            except AttributeError:
                continue
            gen_candidates = _pop_to_candidates(site, profile, fixed, pop)
            generation_callback(gen_idx + 1, gen_candidates)

    # Final pareto front
    if result.opt is not None:
        all_candidates = _pop_to_candidates(site, profile, fixed, result.opt)

    return ParetoResult(candidates=all_candidates, n_gen=opt_params.n_gen)


def _pop_to_candidates(site, profile, fixed, pop) -> list[Candidate]:
    candidates = []
    for ind in pop:
        x = ind.X
        f = ind.F
        params = _params_from_x(x)
        try:
            layout = generator.generate(site, profile, params, fixed=fixed)
            layout = place_special_stalls(layout, site, profile)
        except Exception:
            continue
...
def candidate_advantages(candidate: Candidate, pareto: ParetoResult) -> str:
    """Return a human-readable advantages/disadvantages string for *candidate*
    vs. the Pareto-median of the full front."""
    ...
    # Count
    dc = count - med_count
    lines.append(f"{'✚' if dc >= 0 else '−'} {abs(dc):+d} stalls vs median ({count} total)")

    # Area/stall
    da = area - med_area
    lines.append(f"{'−' if da <= 0 else '+'} {abs(da):.1f} m²/stall vs median ({area:.1f} m²/stall)")
    ...
    if walk > 0:
        dw = walk - med_walk
        lines.append(f"{'−' if dw <= 0 else '+'} {abs(dw):.0f} m avg walk vs median ({walk:.0f} m)")
